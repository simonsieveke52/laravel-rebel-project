			...
			<div class="col-12">
				<ul class="list-group">
					...
					@foreach ($products as $product)
						...
						@if ( isset($product->pivot) && is_array($product->pivot->options) )
							@php
						...
						<li class="list-group-item d-flex justify-content-between align-items-center list-group-item border-secondary">
							<a class="min-w-sm-100px text-center text-black" href="{{ route('product.show', $product) }}">
								<img src="{{ asset( Croppa::url($product->main_image, 100, 100, ['resize', 'upscale' => true]) ) }}" class="img-fluid">
							</a>
							<span class="flex-sm-grow-1 px-3 d-none d-sm-block">
								<a class="mb-1 font-weight-normal" href="{{ route('product.show', $product) }}">
									{{ $product->name }}
								</a>
							</span>
							<span class="min-w-sm-100px text-right">
					...
					<li class="list-group-item d-flex justify-content-between align-items-center list-group-item-secondary border-secondary">
						<span class="flex-sm-grow-1 px-2 text-right">
							...
							<table class="table mb-0">
								...
								<tr>
									...
									<td class="p-1 border-0">
										{{ config('cart.currency_symbol') . number_format($order->discount_amount, 2) }}
									</td>
								</tr>
								@endif
								<tr>
