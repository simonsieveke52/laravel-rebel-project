...
@section('page_header')
    <div class="container-fluid">
        ...
        @endcan
        <a  class="btn btn-info" style="margin-top: 2px; border:none;" target="_blank" href="{{ route('export.orders') }}">Export</a>
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <select
                                        ...
                                        <option {{ (request()->get('order_status', '') == 'all' && ! session()->has('order_status')) ? 'selected' : '' }} value="all">All</option>

                                        @foreach ($orderStatues as $status)
                                            <option
                                                @if (
                                                        ( request()->get('order_status') !== 'all' && (int) request()->get('order_status', -1) === (int) $status->id) ||
                                                        ( session('order_status') !== 'all' && (int) session('order_status', -1) === $status->id))
                                                    selected
                                                @endif
                                ...
                                <div class="col-md-2">
                                    ...
                                    <select
                                        ...
                                        <option value="fba" {{ request()->get('order_type') === 'fba' ? 'selected' : '' }}>Has FBA Items</option>
                                    </select>
                                </div>
                                <div class="col-md-2">
                            ...
                            <div class="panel panel-default panel-min-height">
                                ...
                                <div class="panel-body">
                                    <p class="mb-0">
                                        ...
                                        <span class="jq-order-discount-container hide">Discount: $<span class="jq-order-discount"></span> <br></span>
        ...
        $(document).ready(function () {
            ...
            $('#show-order-modal').on('show.bs.modal', function (event) {
                ...
                $.post( @json(route('voyager.show.order')), {id: order_id, _token: $('meta[name="csrf-token"]').attr('content') }, function(response) {
                    ...
                    if (Number(response.order.discount) > 0) {
                        modal.find('.jq-order-discount').text(response.order.discount)
                        modal.find('.jq-order-discount-container').removeClass('hide')
                    } else {
                        modal.find('.jq-order-discount-container').addClass('hide')
                    }
